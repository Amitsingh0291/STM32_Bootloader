...
uint32_t fw_total_size=0,fw_recieved_size=0;

uint16_t etx_receive_chunk( uint8_t *buf, uint16_t max_len );
ETX_OTA_EX_ etx_process_data( uint8_t *buf, uint16_t len );
HAL_StatusTypeDef write_data_to_flash_app( uint8_t *data,uint16_t data_len, bool is_first_block );
	...
	switch(ota_state){
		...
		case ETX_OTA_STATE_DATA:
		{
			...
			if(dat->packet_type == ETX_OTA_PACKET_TYPE_DATA){
				...
				ex = write_data_to_flash_app( buf+4, dat_len, ( fw_recieved_size == 0) );

				printf("Received OTA DATA\n");
				if( ex == HAL_OK )
				  {
					printf("[%ld/%ld]\r\n", fw_recieved_size/ETX_OTA_DATA_MAX_SIZE, fw_total_size/ETX_OTA_DATA_MAX_SIZE);
...
HAL_StatusTypeDef write_data_to_flash_app( uint8_t *data,uint16_t data_len, bool is_first_block )
{
	...
	for(int i = 0; i < data_len; i++ )
	{
	  ret = HAL_FLASH_Program( FLASH_TYPEPROGRAM_BYTE,
							   (ETX_APP_FLASH_ADDR + fw_recieved_size),
							   data[i]
							 );
	  if( ret == HAL_OK )
	  {
